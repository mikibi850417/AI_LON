...
const UserSettingsForm = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // --- 추가: competitor_hotels 필드가 변경된 경우 FastAPI API 호출 (LocalDB 업데이트) ---
      if (JSON.stringify(selectedCompetitorHotels) !== JSON.stringify(originalCompetitorHotels)) {
        try {
          const competitorResponse = await fetch(`${apiBaseUrl}/api/competitor-hotels/update`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
          ...
          console.log("경쟁 호텔 크롤링 명령 성공:", competitorData);
        } catch (apiError: any) {
          console.error("경쟁 호텔 API 호출 실패:", apiError);
          setError(`경쟁 호텔 API 호출 실패: ${apiError.message || '네트워크 오류'}`);
          setTimeout(() => setError(""), 3000);
          return;
        }
      }
